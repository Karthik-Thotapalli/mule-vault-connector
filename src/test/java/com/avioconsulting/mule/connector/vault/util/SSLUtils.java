package com.hackmatters.mule.connector.vault.util;
...
public class SSLUtils {
    ...
    public static void createClientCertAndKey() throws KeyStoreException, IOException, CertificateException,
        ...
        final FileReader fileReader = new FileReader(VaultContainer.CERT_PEMFILE);
        ...
        try (final FileOutputStream keystoreOutputStream = new FileOutputStream(VaultContainer.CLIENT_TRUSTSTORE)) {
            trustStore.store(keystoreOutputStream, VaultContainer.CLIENT_KEYSTORE_PASSWORD.toCharArray());
        }
        ...
        keyStore.load(null);
        keyStore.setKeyEntry("privatekey", keyPair.getPrivate(), VaultContainer.CLIENT_KEYSTORE_PASSWORD.toCharArray(), new Certificate[]{clientCertificate});
        keyStore.setCertificateEntry("cert", clientCertificate);
        try (final FileOutputStream keystoreOutputStream = new FileOutputStream(VaultContainer.CLIENT_KEYSTORE)) {
            keyStore.store(keystoreOutputStream, VaultContainer.CLIENT_KEYSTORE_PASSWORD.toCharArray());
        }

        // Also write the client certificate to a PEM file, so it can be registered with Vault
        writeCertToPem(clientCertificate, VaultContainer.CLIENT_CERT_PEMFILE);
        writePrivateKeyToPem(keyPair.getPrivate(), VaultContainer.CLIENT_PRIVATE_KEY_PEMFILE);
    }
